...
class RGBImage:
    ...
    def __init__(self, pixels):
        ...
        if not isinstance(pixels,list) or not pixels:
            raise TypeError()
        elif not all(isinstance(row, list) and row for row in pixels):
            raise TypeError()
        elif not all(len(row) == len(pixels[0]) for row in pixels):
            raise TypeError()
        elif not all(isinstance(p, list) for row in pixels for p in row):
            raise TypeError()
        elif not all(len(p) == 3 for row in pixels for p in row):
            raise TypeError()
        elif not all(0 <= x <= 255 for row in pixels for pixel in row for x in pixel):
            raise ValueError()
    ...
    def get_pixels(self):
        ...
        """
        # YOUR CODE GOES HERE #
        return [[[ele for ele in inner] for inner in outer] for outer in self.pixels]
    ...
    def set_pixel(self, row, col, new_color):
        ...
        if not isinstance(row, int) or not isinstance(col, int):
            ...
            raise TypeError()
        elif not len(new_color)==3:
            raise TypeError()
        elif not all(isinstance(num, int) for num in new_color):
            raise TypeError()
        elif any(num > 255 for num in new_color):
            raise ValueError()
...
class ImageProcessingTemplate:
    ...
    def negate(self, image):
        ...
        """
        # YOUR CODE GOES HERE #
        return RGBImage([[[255 - val for val in inner] for inner in outer] for outer in image.get_pixels()])
    ...
    def grayscale(self, image):
        ...
        """
        # YOUR CODE GOES HERE #
        return RGBImage([[[sum(inner)//3, sum(inner)//3, sum(inner)//3] for inner in outer] for outer in image.get_pixels()])
    ...
    def rotate_180(self, image):
        ...
        """
        # YOUR CODE GOES HERE #
        return RGBImage(list(map(lambda x: x[::-1], (ls for ls in image.get_pixels())))[::-1])
    ...
    def get_average_brightness(self, image):
        ...
        """
        # YOUR CODE GOES HERE #
        total_brightness = sum([sum([sum(inner)//3 for inner in outer]) for outer in image.get_pixels()])
        total_pixels = image.num_cols * image.num_rows
    ...
    def adjust_brightness(self, image, intensity):
        ...
        if not isinstance(intensity, float):
            raise TypeError()
        return RGBImage([[[min(255, max(0, int(val * intensity))) for val in inner] for inner in outer] for outer in image.get_pixels()])
...
class PremiumImageProcessing(ImageProcessingTemplate):
    ...
    def pixelate(self, image, block_dim):
        ...
        new_pixels = [[pixels[r][c][:] for c in range(cols)] for r in range(rows)]

        for i in range(0, rows, block_dim):
            for j in range(0, cols, block_dim):
             
                block = [pixels[x][y] for x in range(i, min(i + block_dim, rows))
                                         for y in range(j, min(j + block_dim, cols))]
                avg_r = sum(p[0] for p in block) // len(block)
                avg_g = sum(p[1] for p in block) // len(block)
                avg_b = sum(p[2] for p in block) // len(block)

                for x in range(i, min(i + block_dim, rows)):
                    for y in range(j, min(j + block_dim, cols)):
    ...
    def edge_highlight(self, image):
        ...
        """
        # YOUR CODE GOES HERE #
        single_pixel = [[sum(element) // 3 for element in row] for row in image.pixels]
        single_copy = [[sum(element) // 3 for element in row] for row in image.pixels]
        ...
        for r in range(rows):
            for c in range(cols):
                ...
                pixel = max(0, min(255, (single_pixel[r][c] * 8) - sum(surround)))
                single_copy[r][c] = [pixel, pixel, pixel]

        return RGBImage(single_copy) 
...
class PremiumPlusMultimediaProcessing(PremiumImageProcessing):
    ...
    def slow_down(self, audio, factor):
        ...
        return AudioWave([x for x in audio.wave for i in range(factor) ])
    ...
    def reverb(self, audio):
        ...
        return AudioWave(audio.wave[:4] + [
        max(-32768, min(32767, round(audio.wave[i] + sum(audio.wave[i - j] * w \
        for j, w in enumerate([0.8, 0.6, 0.4, 0.2], 1))))) for i in range(4, len(audio.wave))])
    ...
    def clip_song(self, audio, start, end):
        ...
        return AudioWave(audio.wave[int(len(audio.wave)*(start/100)): int(len(audio.wave)*(end/100)) +1])
...
class ImageKNNClassifier:
    ...
    def distance(self, image1, image2):
        ...
        squared_diffs = sum([sum([sum([(p1[i][j][k] - p2[i][j][k])**2 for k in range(3)])for j in range(image1.num_cols)])for i in range(image1.num_rows)])
    ...
    def vote(self, candidates):
        ...
        """
        # YOUR CODE GOES HERE #
        freq = {x: 0 for x in set(candidates)} 
        for i in candidates:
            freq[i] += 1
    ...
    def predict(self, image):
        ...
        distance = list(map (lambda x,y: (self.distance(x[0], y), x[1]), self.data, [image for i in range(len(self.data))]))
        nearest = sorted(distance, key=lambda x: x[0], reverse=False)[:self.kn]
        candidates = [x[1] for x in nearest]
